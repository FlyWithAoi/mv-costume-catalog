...
- ログイン不要でURLから閲覧できる
- アイドル名・ユニット名・衣装名・タグで検索できる（別名・表記ゆれ対応）
  - ユニット名（例: `fine` / `フィーネ`）で検索すると、そのユニット所属アイドルの衣装がすべてヒットします
- 事務所・ユニット・アイドルの連動プルダウンで衣装に絞り込める（選択肢は各マスタから自動生成）
- 解放状態で絞り込める
- 初期表示は「リクエスト可能な衣装のみ」
...
mv-costume-catalog/
  public/                       ← 公開対象（GitHub Pagesで配信するのはここ）
    ...
    app.js
    data/
      costumes.json             ← 衣装データ（手入力。idol_slug でアイドルを参照）
      idols.json                ← アイドルマスタ（表示名・読み・ローマ字・別名）
      units.json                ← ユニットマスタ（表示名・別名・所属メンバー）
      offices.json              ← 事務所マスタ（事務所ごとのidol・unit所属）
    images/
      costumes/
